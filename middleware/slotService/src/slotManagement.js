...
async function update_slot(message,client){
    try{
        var[time,date,referenceCode] = await Promise.all([
            ...
            validation.validate_date(message),
            /*
            // validation.validate_reference_code(jsonMessage.referenceCode),
            // validation.validate_dentist(jsonMessage.dentist),
        ...
        if(time && date && referenceCode){
            ...
            const payload = {
                _id: jsonMessage._id, // MongoDB ID
